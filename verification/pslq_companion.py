...
def const_library():
    ...
    C = {
        ...
        "pi*e": mp.pi * mp.e,
        "pi+e": mp.pi + mp.e,
        # zeta(2)*zeta(3) and zeta(2)+zeta(3)
        "zeta2*zeta3": (mp.pi ** 2 / 6) * mp.zeta(3),
        "zeta2+zeta3": (mp.pi ** 2 / 6) + mp.zeta(3),
    }
    _CONST_CACHE[key] = C
    return C
...
def pslq_identify(L, maxcoeff, maxsteps):
    ...
    for mix, nm in [([one, L, C["pi^3"], C["zeta3"]], ["1", "L", "pi^3", "zeta3"]),
                    ...
                    # pi/e linear combinations and products
                    ([one, L, C["pi"], C["e"]], ["1", "L", "pi", "e"]),
                    ([one, L, C["pi*e"]], ["1", "L", "pi*e"]),
                    ([one, L, C["pi+e"]], ["1", "L", "pi+e"]),
                    # zeta(2)*zeta(3) and zeta(2)+zeta(3)
                    ([one, L, C["zeta2*zeta3"]], ["1", "L", "zeta2*zeta3"]),
                    ([one, L, C["zeta2+zeta3"]], ["1", "L", "zeta2+zeta3"]),
                    # euler-gamma combinations
                    ([one, L, C["euler_gamma"], C["pi"]], ["1", "L", "euler_gamma", "pi"]),
                    ([one, L, C["euler_gamma"], C["log2"]], ["1", "L", "euler_gamma", "log2"])]:
        h = rel(mix, nm)
        if h:
